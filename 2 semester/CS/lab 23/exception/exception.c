...
void sxAddTraceEntry(const struct SxTraceEntry entry) {
  // trace[0] is the first queue frame - it has initiated the exception.
...
void _sxLeaveTry(const char *file, int line) {
  ...
  //-001-  -rbtree_val-!E-  (no CATCH - no throw())
  // 010    TCE     n/a         unreach.  n/a     n/a     LJC=0 C=0
  //-011-  -TC!E-  (CATCH cannot be reached without a throw() in TRY)
  // 100    rbtree_val!E     LJC>0 C=0   n/a       n/a     n/a     LJC>0 C=0 >> rethrow
  //-101-  -rbtree_val!-!E- (no CATCH - no throw())
  // 110    rbtree_val!CE    LJC>0 C=0   LJC=0 C=1 n/a     n/a     LJC=0 C=1
  // 111    rbtree_val!C!E   LJC>0 C=0   LJC=0 C=1 LJC>0 C=1} LJC>0 C=2} LJC>0 C=2 >> r.
  //
  // FINALLY reduction:
